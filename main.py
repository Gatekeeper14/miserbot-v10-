    ...
    if "name" not in user:
        ...
        try:
            ai = client_ai.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a friendly business assistant. Keep replies short and human."},
                    {"role": "user", "content": f"User name is {user['name']}, service is {user['service']}. Write a short confirmation message."}
                ]
            )
